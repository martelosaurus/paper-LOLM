...
X[, valuation.adj := valuation - amount] ### Check that valuation and amount reported for all rounds
...
#X = X[prior.valuation.adj>0] 
X = X[prior.cum.Return>0] # Need to check why some values are 0
#X[,ln_Return := log(valuation.adj/prior.valuation.adj)]
X[,ln_Return := log(cum.Return/prior.cum.Return)] # check this #s seem low  LEFT OFF HERE
